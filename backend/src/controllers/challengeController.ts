...
import Challange from '../models/Challenge';
...
export const deleteChallenge = async (req: Request, res: Response) => {
  try {
    const challange = await Challange.findByIdAndDelete(req.params.id);
    if(!challange)
...
export const updateChallenge = async (req: Request, res: Response) => {
  try {
    const challange = await Challange.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
